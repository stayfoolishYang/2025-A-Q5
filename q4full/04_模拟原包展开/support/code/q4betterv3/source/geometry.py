...
def _deduplicate_cycle(poly):
    """Remove only exactly equal neighbours/closure; retain every distinct point.

    No distance tolerance is used: merging merely close vertices could remove an
    extreme point and shrink the feasible outer polygon.
    """
    points = np.asarray(poly, dtype=float).reshape(-1, 2)
    if len(points) < 2:
        return points.copy()
    keep = np.r_[True, np.any(points[1:] != points[:-1], axis=1)]
    points = points[keep]
...
def clip(poly, normal, bound):
    ...
    inside = d <= 1e-9
    if inside.all():
        return _deduplicate_cycle(poly)
    if not inside.any():
        return np.empty((0, 2))
    result = []
    ...
    return _deduplicate_cycle(result)
...
def diameter(poly):
    """FP64 vertex-pair diameter, robust to duplicates and degenerate ordering.

    For a convex polygon its diameter is attained by two vertices. Enumerating
    all pairs avoids the strict-convexity assumptions of rotating calipers.
    Cost is O(n**2) arithmetic and O(n) additional memory. No near vertex is
    discarded. Floating-point distances are numerical values, not interval
    arithmetic certificates; clearance retains its separate vertex checks.
    """
    points = np.asarray(poly, dtype=float)
    if points.size == 0:
        raise ValueError('Empty feasible region')
    if points.ndim != 2 or points.shape[1] != 2 or not np.isfinite(points).all():
        raise ValueError('Expected finite vertices with shape (n, 2)')
    best, pair = -1., (0, 0)
    for i, point in enumerate(points):
        distances = np.hypot(points[i:, 0]-point[0], points[i:, 1]-point[1])
        j = int(np.argmax(distances))
        value = float(distances[j])
        if value > best:
            best, pair = value, (i, i+j)
    return best, points[pair[0]].copy(), points[pair[1]].copy()

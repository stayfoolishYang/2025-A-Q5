...
def main():
    ...
    if a.problem==3:
        ...
        hashes=json.loads((root/'cpu519/PLAN.json').read_bytes())['hashes']
        commit='79766ecf0d2577289f814edba310327f55424111'
    else:
        root=BASE/'q4_latest_7228633';src=root/'source/q4better'
        hashes=json.loads((root/'cpu519/HASHES.json').read_bytes())
        commit='7228633906262bb035f4ea1d859792e9f4484d20'
    checked={name:h for name,h in hashes.items() if Path(name).is_relative_to(src)}

...
def fingerprints(sim):
    files=list((ROOT/'source').rglob('*.py'))+list((ROOT/'offline_engine').glob('*.py'))
...
def run_scene(scene_path,output):
    ...
    rb.save(output/'manifest.json',dict(records=[rec],configs=[cfg],source_hashes=fingerprints(ROOT/'offline_engine'),execution_backend='LOCAL_DEV',execution_purpose='FRAMEWORK_INTEGRATION',production_eligible=False))
    ...
    try:row=base.run_case((output,ROOT/'offline_engine',0,'B',None))

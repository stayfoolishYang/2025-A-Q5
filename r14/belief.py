...
class DiscoveryBelief:
    ...
    def observe(self, channel, point, result):
        ...
        if result != 'no_signal':
            ...
            if key not in self.negative_points[channel]:
                self.masks[channel] &= ~detects(self.states, [point])[0]
    ...
    def predict(self, nodes, conditioned=True):
        ...
        hits = detects(self.states, nodes)
        ...
        masses = np.array([(hits & mask).mean(axis=1) for mask in masks])
        z = evidence(likelihood, len(self.seen))
        if z <= 0 or any(likelihood == 0):
        ...
        return dict(available=True, channels=channels, expected_new=unconditional.sum(axis=0),
                    ...
                    conditional=np.divide(masses, likelihood[:, None]), particles=len(self.states))
    ...
    def survival(self, nodes):
        ...
        out = [1.]
        for hit in detects(self.states, nodes):
            masks &= ~hit

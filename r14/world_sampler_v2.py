...
class WorldSampler:
    ...
    def sample(self,seed):
        rng=np.random.default_rng(seed);sources=[];fields={};proposals=0;rejections=[]
            ...
            for batch in range(8):
                ...
                for particle in particles[:32]:
                    ...
                    try:field=PairedNoise(particle[:2],self.hist[c],int(rng.integers(2**63)))
                        ...
                        rejections.append(dict(channel=c,kind='directional' if particle[4] else 'omni',reason=str(e),history_length=len(self.hist[c])))
                        continue
        ...
        return engine,dict(proposals=proposals,sampled_count=len(sources),history_actions=len(self.actions),noise_rejections=rejections,source_kinds={c:s.kind for c,s in engine.sources.items()})

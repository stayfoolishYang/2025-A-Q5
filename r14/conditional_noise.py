...
class ConditionalNoise:
    def __init__(self,source_position,history,seed):
        ...
        for point,result,angle in history:
            ...
            base=np.degrees(np.arctan2(delta[1],delta[0]))%360
            error=(angle-base+180)%360-180
            # Strict interior of rounding bin; physically bounded bearing error.
            lo=max(-1.,error-.005+1e-9);hi=min(1.,error+.005-1e-9)

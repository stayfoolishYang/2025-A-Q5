...
class StagedWeightedWorldSampler(WorldSampler):
    def sample(self,seed,source_count=128,noise_count=256):
        ...
        for c in sorted(self.known):
            ...
            for batch in range(64):
                ...
                if batch+1 in (16,32,64):checkpoints.append(dict(attempts=(batch+1)*4096,valid=len(proposals)))
                if len(proposals)>=source_count:break
            raw_attempts=(batch+1)*4096;valid_count=len(proposals)
            proposals=np.array(proposals[:source_count])
            if len(proposals)<source_count:
                raise SamplingUnavailable(f'Incomplete source proposal bank: channel={c}, attempts={raw_attempts}, valid={valid_count}, cap=262144')
            banks=[];logs=[];stats=[]
            ...
            diagnostics[c]=dict(raw_attempts=raw_attempts,valid_count=valid_count,acceptance_rate=valid_count/raw_attempts,extended_budget=raw_attempts>65536,checkpoints=checkpoints,source_ess=float(1/(w@w)),source_proposals=len(proposals),noise_samples=noise_count,selected_noise_ess=stats[selected]['ess'],zero_source_weights=sum(not s['positive'] for s in stats))
...
from unknown_discovery import DiscoveryWorldSampler
class FinalizationSampler(DiscoveryWorldSampler):
    sample=StagedWeightedWorldSampler.sample

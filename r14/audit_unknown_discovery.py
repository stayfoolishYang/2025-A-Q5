...
def worker(job):
 ...
 seed=int.from_bytes(hashlib.sha256(f'Q1U:{i}:{j}:{bank}'.encode()).digest()[:8],'big')
 ...
 try:
  ...
  result=dict(status='FUNCTIONAL_READY',known=len(s.known),classes=len(u.groups),p_new=u.pnew.tolist(),M=u.m.tolist(),pm=u.pm.tolist(),top3=order[:3].tolist(),gap3=float(u.pnew[order[2]]-u.pnew[order[3]]) if len(order)>3 else None,groups=[dict(channels=cs,history_length=len(h),L=u.data[h]['L']) for h,cs in u.groups.items()])
  try:
   world,meta=s.sample(seed,128,512);result['world_replay']='PASS';result['sampled_count']=len(world.sources)
...
if __name__=='__main__':
 ...
 (OUT/'preregistration.json').write_text(json.dumps(dict(states=64,banks=3,sobol_points=4096,probability_fraction_within_002=.95,max_probability_range=.05,top3_agreement=.95,max_count_total_variation=.02,world_replay_required='192/192',no_state_replacement=True,Q2='not run in this audit'),indent=2))

...
from plot_style import paper_style,MATLAB_COLORS
...
def main():
    ...
    with paper_style(font='Times New Roman',overrides={'font.family':['Times New Roman','SimSun'],'font.size':9,'axes.labelsize':9,'xtick.labelsize':8,'ytick.labelsize':8,'legend.fontsize':8,'axes.unicode_minus':False}),PdfPages(O/'R13_13张机制与效果图谱.pdf') as pdf:
        ...
        for key,g in c.groupby('next_action'):ecdf(aa[0],g.scan_credit_s,{'certified_clear':'保证清除','discovery':'发现扫描','active_localization':'主动定位','diagnostic':'诊断'}.get(key,key),{'discovery':'#555555','certified_clear':color('C2'),'active_localization':color('A1'),'diagnostic':color('C1')}.get(key,'#444444'))
        ...
        g=p[(p.method=='D')&(p.N==16)];sc=aa[1].scatter(g.baseline_post_known16_s,g.post_known16_s,c=g.delta_s,cmap='coolwarm',s=30,vmin=-max(abs(g.delta_s).max(),1e-9),vmax=max(abs(g.delta_s).max(),1e-9));fig.colorbar(sc,ax=aa[1],label='耗时差 (s/源)');lim=max(g.baseline_post_known16_s.max(),g.post_known16_s.max());aa[1].plot([0,lim],[0,lim],color='.5',lw=.8);aa[1].set_xlabel('基线剩余时间 (s)');aa[1].set_ylabel('软释放剩余时间 (s)');save(fig,8,'确认上限后的完成效率',pdf)
        fig,aa=axes(2,9);aa=aa[0]
        for m in methods:
        ...
        fig,aa=axes(2,11);aa=aa[0];case=int(cg.loc[cg.delta_s.idxmin(),'id'])
        max_action=max(sum(a['position'] is not None for a in loadtrace(cohort,m,case)['actions']) for m in ['R12',candidate])-1
        for ax,m in zip(aa,['R12',candidate]):
            tr=loadtrace(cohort,m,case);pts=np.array([a['position'] for a in tr['actions'] if a['position'] is not None]);ax.plot(pts[:,0],pts[:,1],color=color(m),lw=.65,alpha=.8,label=lab(m));ax.add_patch(Circle((0,0),1800,fill=False,edgecolor='.3',lw=.7));nodes=np.array(tr['route_events'][0]['nodes']);ax.scatter(nodes[:,0],nodes[:,1],marker='s',s=12,facecolors='none',edgecolors='.4',label='保证节点');sc=ax.scatter(pts[:,0],pts[:,1],c=np.arange(len(pts)),s=6,cmap='viridis',alpha=.6,vmin=0,vmax=max_action);fig.colorbar(sc,ax=ax,label='动作序号',shrink=.6);ax.set_aspect('equal');ax.set_xlabel('横坐标 (m)');ax.set_ylabel('纵坐标 (m)');ax.legend(fontsize=7);ax.set_xlim(-2300,2300);ax.set_ylim(-2300,2300)
        ...
        fig,aa=axes(2,9);aa=aa[0];h=read('holdout_summary.csv');v=h[h.method==candidate].iloc[0];dev=d[d.method==candidate].iloc[0]
        names=['均值','95%分位','99%分位','最大值']
        for values,offset,label,c in [(dev,-.1,'开发集',color('A1')),(v,.1,'独立验证集',color('C2'))]:
            gain=[values.improvement_pct,-values.P95_change_pct,-values.P99_change_pct,-values.max_change_pct]
            aa[0].scatter(gain,np.arange(4)+offset,label=label,color=c,s=35)
        aa[0].set_yticks(range(4),names);aa[0].axvline(0,color='.5',lw=.7);aa[0].set_xlabel('相对基线改善 (%)');aa[0].legend()
        for cc,label,c in [('development','开发集',color('A1')),('holdout','独立验证集',color('C2'))]:
            pp=read(cc+'_pairs.csv');ecdf(aa[1],pp[pp.method==candidate].delta_s,label,c)
        aa[1].axvline(0,color='.5',lw=.7);aa[1].set_xlabel('逐场耗时差 (s/源)');aa[1].set_ylabel('累计场景比例');aa[1].legend();save(fig,13,'开发收益与独立验证',pdf)
